...
##### BEDOURET BAPTISTE, CELLITTI FRANCESCO & NEGRONI EDOARDO #####
...
#### Tuning for SVM
profvis({tuning = foreach (i = 1:20, .combine = rbind) %dopar%{
  set.seed(i)
  library(MASS)
  library(e1071)
...
# Comparison with non-parallelized functions

log_tuning_NP = matrix(NA,0,ncol = 4)
cir_tuning_NP = matrix(NA,0,ncol = 4)


profvis({tuning_NP = foreach (i = 1:20, .combine = rbind) %do%{
  set.seed(i)
  library(MASS)
  library(e1071)
  X <- mvrnorm(n, mu = mean_vector, Sigma = cov_matriX)
...
colors1 <- ifelse(ds['y1']==0, "firebrick", "orange")
colors2 <- ifelse(ds['y2']==0, "firebrick", "orange")

library(ggplot2)
ggplot(data = ds, mapping = aes(x = ds[,103], y = ds[,104]))+
  geom_point(col = colors2) +
  xlab('First Principal Component') +
  ylab('Second Principal Component') +
  theme_minimal()

ggplot(data = ds, mapping = aes(x = ds[,3], y = ds[,4]))+
  geom_point(col = colors1) +
  xlab('First dimension of X') +
  ylab('Second Dimension of X') +
  theme_minimal()
...
profvis({parallelo = foreach (i = 20:1020, .combine = rbind) %dopar%{
...
summary(parallelo)
errors = apply(parallelo, 2, sd)

parallelo1 = data.frame(parallelo)
colnames(parallelo1) = c('SVM linear','Random Forest linear', 'SVM circular', 'Random Forest circular')
ggplot(stack(parallelo1), mapping = aes( x =ind, y = values)) +
  geom_boxplot(color = 'black', fill = 'darkorange') +
  xlab('') +
  ylab('Miss classification error') +
  theme_linedraw() +
  scale_fill_brewer(palette = 1) +
  title('Miss classification error for non-parametric classification')
